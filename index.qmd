...
## PLAY demographics {.scrollable}
  
```{r}
#| label: fig-play-demog-race
#| fig-cap: "Reported race using OMB categories for children in PLAY sample."
#| echo: true
play_summ_demog_df |>
  filter(!is.na(race)) |>
  mutate(n_race = fct_rev(fct_infreq(race))) |>
  ggplot() +
  geom_col(aes(x = n_race, fill = n_race), stat = "count") +
  coord_flip() +
  theme_classic() +
...
- Code
  ...
  - Analysis
- Data
  - Non-identifiable^[But consider AI, non-expected uses.]
...
- Identifiable data
  ...
  - Geographic information (e.g., address-related) for contextual info
    - Median income, health, education, environmental quality, climate, etc.^[e.g., Was your sample collected in an area with high COVID rates?]
...
- Strengthen open practices into your lab group
